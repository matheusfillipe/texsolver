...
def solveEq(sol, sym):
    if len(sol)>0:
        if len(sol)==1: printex(sym+r"="+latex(sol[0]))
        else:
...
def texSolve(eq):
    try:
        ...
        "Invalid expression! Please use [math latex](https://en.wikibooks.org/wiki/LaTeX/Mathematics)"
        return
    ...
    elif type(se)==sympy.core.relational.Equality:
